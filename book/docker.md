...
#### **Интеграция Docker с системами мониторинга (Prometheus, Grafana и др.)**:
...
#### **Изоляция контейнеров через AppArmor и SELinux (дополнительные меры безопасности)**:
...
#### Продвинутые сетевые конфигурации:
...
#### Пример .dockerignore
```
# Игнорируем директорию системы контроля версий Git
.git/

# Игнорируем директорию с зависимостями Node.js
node_modules/

# Исключаем логи, которые создаются при запуске приложения
*.log

# Исключаем файлы конфигурации окружения, которые могут содержать чувствительные данные
.env

# Игнорируем директории сборки и артефактов
dist/        # финальная сборка фронтенда
build/       # директория сборки

# Игнорируем временные файлы и кэш редакторов и операционной системы
.vscode/
.idea/
.DS_Store    # на macOS

# Игнорируем файлы и директории с результатами тестов
coverage/

# Игнорируем документацию и файлы офисных приложений
*.doc
*.pdf

# Исключаем архивы и пакеты, которые могут создаваться для сборки проекта
*.zip
*.tar.gz

# Исключаем временные и резервные файлы
*.tmp
*.swp
*.bak

# Включаем важные файлы, несмотря на общее правило выше
!important.txt  # несмотря на правило *.txt, этот файл будет включен

# Исключаем Docker-override файлы для локальных конфигураций
...
Запуск Docker-контейнеров без прав суперпользователя на хосте осуществляется с помощью **Rootless Docker**.

### Шаги для запуска контейнеров не из-под рута:

1. **Установите Rootless Docker**:
   - Сначала необходимо установить Docker в режиме rootless. Для этого вам нужно выполнить команду на хостовой машине:
     ```
     curl -fsSL https://get.docker.com/rootless | sh
     ```

2. **Настройте окружение пользователя**:
   - После установки вам нужно добавить переменные окружения в ваш `.bashrc` или `.zshrc` (в зависимости от используемого шелла):
     ```
     export PATH=/home/your_user/.docker/bin:$PATH
     export DOCKER_HOST=unix:///run/user/$(id -u)/docker.sock
     ```

3. **Перезапустите терминал** или выполните `source ~/.bashrc` или `source ~/.zshrc`, чтобы применить изменения.

4. **Проверьте, что Docker работает в rootless режиме**:
   - Запустите команду для проверки статуса Docker:
     ```
     docker info | grep Rootless
     ```
   - Если установка прошла успешно, вы увидите вывод: `Rootless: true`.

5. **Запускайте контейнеры как обычный пользователь**:
   - Теперь можно запускать контейнеры без использования root-доступа:
     ```
     docker run -d --name mycontainer nginx
     ```

### Особенности Rootless Docker:
- **Безопасность**: Контейнеры работают от имени обычного пользователя и не могут получить доступ к хостовой системе как root. Это увеличивает безопасность, особенно в многоарендных системах.
- **Ограничения**: Rootless Docker может иметь некоторые ограничения по производительности и поддержке некоторых системных функций (например, сетевых интерфейсов с режимом `host`).

Rootless Docker — это отличный способ повысить безопасность контейнеров, запуская их без привилегий суперпользователя.
