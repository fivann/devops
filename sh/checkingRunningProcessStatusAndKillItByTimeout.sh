...
timeout=1800 # 30 minutes in seconds
start_time=$(date +%s)
admin_email="admin@myproject.net"
...
while true; do
    ...
    if [ "$status" == "failed" ]; then
        echo "failed!" | mail -s "Checkbackups status: Failed" $admin_email
        exit 1
    elif [ "$status" == "done" ]; then
        echo "done!"
        exit 0
        ...
        current_time=$(date +%s)
        if [ $((current_time - start_time)) -gt $timeout ]; then
            echo "timeout, terminating" | mail -s "Checkbackups status: Timeout" $admin_email
            pkill checkbackups
            exit 1
        fi
    fi
